        ...
        <SectionWrapper id="experience">
            ...
            <div className="mt-12 space-y-0">
                {experience.map((exp, i) => (
                    <div key={i} className="relative border-l border-white/25 py-6 pl-8 last:pb-0">
                        <span className={`absolute -left-1.25 top-8 h-2.5 w-2.5 rounded-full ${exp.current ? "bg-ember" : "bg-signal"}`} />
                        <div className="flex flex-col justify-between gap-1 sm:flex-row sm:items-baseline sm:gap-2">
                            <div>
                                <h3 className="font-display text-lg font-semibold text-paper">{exp.role}</h3>
                                <h4 className="font-display text-md font-medium text-fog">{exp.company}</h4>
                            </div>
