...
function CopyEmailButton() {
    ...
    return (
        <button
            ...
            className="rounded-md p-1.5 text-fog transition-colors hover:text-signal-light"
        >
...
export function Contact() {
    ...
    return (
        <SectionWrapper id="contact">
            <p className="mb-2 font-mono text-sm text-ember">{"// 05 contact"}</p>
            <h2 className="font-display text-3xl font-semibold text-paper md:text-4xl">
                Let&apos;s talk
            </h2>
            <p className="mt-4 max-w-xl text-fog">
                Got a project in mind, an opportunity, or just want to talk shop about
                dev or AI? Reach out.
            </p>

            <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3 md:grid-rows-2">
                {/* Form — celda grande, ocupa 2x2 */}
                <form
                    onSubmit={handleSubmit}
                    className="space-y-4 rounded-lg border border-white/10 bg-surface p-6 md:col-span-2 md:row-span-2"
                >
                    <div>
                        <label htmlFor="name" className="mb-1 block font-mono text-xs text-fog">
                            Name
                        </label>
                        <input
                            id="name"
                    ...
                    <div>
                        <label htmlFor="email" className="mb-1 block font-mono text-xs text-fog">
                            Email
                        </label>
                        <input
                            id="email"
                    ...
                    <div>
                        <label htmlFor="message" className="mb-1 block font-mono text-xs text-fog">
                            Message
                        </label>
                        <textarea
                            id="message"
                    ...
                    <button
                        ...
                        className="rounded-md bg-ember px-6 py-3 font-medium text-ink transition-colors hover:bg-ember/90 disabled:opacity-50"
                    >
                        {status === "sending" ? "Sending..." : "Send message"}
                    </button>

                    {status === "sent" && (
                        <p className="text-sm text-signal-light">Message sent! I&apos;ll get back to you soon.</p>
                    )}
                    {status === "error" && (
                ...
                {/* Email — celda propia con botón de copiar */}
                <div className="flex flex-col justify-between rounded-lg border border-white/10 bg-surface p-5">
                    ...
                    <div>
                        <p className="font-mono text-xs text-fog">Email</p>
                        <p className="mt-1 truncate text-sm text-paper/90">{EMAIL}</p>
                    </div>
                </div>

                {/* GitHub + LinkedIn — celda compartida */}
                <div className="flex flex-col justify-center gap-4 rounded-lg border border-white/10 bg-surface p-5">
                    <a
                        href="https://github.com/JiaChengZhang14"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-3 text-fog transition-colors hover:text-signal-light"
                    >
