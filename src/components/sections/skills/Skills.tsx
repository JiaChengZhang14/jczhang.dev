            ...
            <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                {skills.map((group) => (
                    <div key={group.category}>
                        <h3 className="font-mono text-xs uppercase tracking-wider text-signal-light">{group.category}</h3>
                        <div className="mt-4 grid grid-cols-3 gap-4">
                            {group.items.map(({ name, icon: Icon }) => (
                                <div key={name} className="flex flex-col items-center gap-2 rounded-lg border border-white/10 bg-surface p-3 text-center">
